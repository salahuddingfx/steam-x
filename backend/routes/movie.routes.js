  ...
  try {
    ...
    if (archiveUrl) {
      responseData.playback = {
        ...
        streamCandidates: [
          archiveUrl,
          `/api/proxy/stream?url=${encodeURIComponent(archiveUrl)}`,
        ],
      };
      responseData.legalSource = 'Internet Archive';
    } else {
      // Fallback to VidSrc (always available) + Watchmode legal providers
      const vidsrcUrl = `https://vidsrc.to/embed/${responseData.type === 'tv' ? 'tv' : 'movie'}/${movie.tmdbId}`;
      responseData.playback = {
        mode: 'vidsrc-fallback',
        source: 'VidSrc (Multiple Sources)',
        adFree: false,
        streamUrl: vidsrcUrl,
        streamCandidates: [vidsrcUrl],
      };
      responseData.legalSource = watchmodeData.success ? 'Available on ' + watchmodeData.providers.map(p => p.name).slice(0, 2).join(', ') : null;
    }
