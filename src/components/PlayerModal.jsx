...
import { FiPlay, FiPause, FiX, FiMaximize2, FiVolume2, FiSettings } from 'react-icons/fi'
...
export default function PlayerModal() {
  ...
  const [server, setServer] = useState('server4')
      ...
      >
        ...
        <div className="w-full h-full flex items-center justify-center bg-black">
         ...
         {isPlaying ? (
            <div className="w-full h-full relative">
                ...
                {/* Server Selector (Top Right Overlay) */}
                <div className={`absolute top-20 right-4 flex flex-col gap-2 transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0'} z-50`}>
                    <div className="bg-black/80 backdrop-blur-md p-2 rounded-lg border border-gray-800">
                        <p className="text-xs text-gray-400 mb-2 font-bold uppercase tracking-wider">Stream Quality</p>
                        <div className="flex flex-col gap-1">
                            {[
                                {id: 'server1', name: 'Ultra HD (AutoEmbed)'},
                                {id: 'server2', name: 'Fast Server (VidSrc VIP)'}, 
                                {id: 'server3', name: 'Backup (VidSrc CC)'},
                                {id: 'server4', name: 'Legacy Pro'}, 
                                {id: 'server5', name: 'SuperEmbed'}
                            ].map((srv) => (
                                <button
                                    key={srv.id}
                                    onClick={() => { setIsBuffering(true); setServer(srv.id); }}
        ...
        <div className={`absolute top-0 left-0 right-0 p-4 bg-gradient-to-b from-black/80 to-transparent transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0'}`}>
          <div className="flex justify-between items-center max-w-7xl mx-auto">
             <h3 className="text-white font-medium text-lg truncate shadow-black drop-shadow-md">{selectedMovie.title}</h3>
             <button
              onClick={handleClose}
        ...
        {showSubtitles && isPlaying && (
          <div className="absolute bottom-32 left-0 right-0 text-center text-white font-semibold text-lg">
            <p className="bg-black bg-opacity-50 inline-block px-3 py-1 rounded">
              {formatTime(playerTime)} - Watch time display
            </p>
          </div>
        )}

        {/* Controls - Show on hover */}
        {/* Only show custom controls if we are NOT using an iframe embed, or strictly for the top bar */}
        {/* We keep the Top Bar active for allowing Exit */}
        {/* We hide the Bottom Control Bar because embeds have their own controls */}
        { tmdbId && !isNaN(tmdbId) ? null : (
        <div
